...
- **Matchday-kaart**: `shareMatchdayCard()` deelt de wedstrijdaankondiging. De aanwezigheidslijst is `cm.squad` — geen apart veld, zodat een late aanmelding gewoon een tik op het selectiescherm is. Aanvangstijd en thuis/uit staan niet op de wedstrijd maar komen uit `srzaData.programma`, gezocht op datum.
