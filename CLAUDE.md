...
- **Matchday**: staat los van de wedstrijdflow, in `DB.matchday` gesleuteld op datum (`{aanwezig, afgemeld, verzameltijd}`). De knop zit bij het programma op Wedstrijden, zodat je kunt aankondigen zonder een wedstrijd aan te maken; `proceedToSquad()` vult de selectie later voor uit die lijst. Wie in geen van beide lijsten staat komt niet op de kaart. `verzameltijd` staat standaard op een kwartier voor aanvang, afgerond op vijf minuten. De aanvangstijd staat niet op de wedstrijd maar komt uit `srzaData.programma`, gezocht op datum.
